...
public class MI {
	...
	private HashMap<String, Double> _featureMI;    //记录所有词的词频，以便用以确定将词频排名靠前的词语提出来作为特征
	...
	private final int _numOfFeature = 100;
	...
	public MI() throws IOException{
        ...
        _featureMI = new HashMap<String,Double>();
        ...
        for(int i = 0; i < tempwordStrings.length; i++){
        	...
        	for(int j = 0; j < _documentClassNameString.length; j++){//计算这个词语对这个类的卡方值同时和当前最大的值进行比较判断是否替换
        		//System.out.println(_documentClassNameString.length);
        		double A = 0;
        		double B = 0;
        		double C = 0;
        		double N = _documentTextHashMap.length;
        		...
        		double tempMI = Math.log((A*N)/((A+C)*(A+B)));
        		//System.out.println("+++++++++++++++++++++++" + tempMI);
        		if(tempMI > maxMI)
        			maxMI = tempMI;
        	}//for循环结束后得到词语tempwordStrings[i]到某各类的最大的卡方值，将其存储到_featureCHI中以便后面进行特征提取时做依据
        	
        	_featureMI.put(tempwordStrings[i], maxMI);
        	
        }//for循环结束后_featureMI保存了所有词的互信息，下面进行特征提取
        
        
        List<Map.Entry<String, Double>> list_Data = new ArrayList<Map.Entry<String, Double>>(_featureMI.entrySet()); 

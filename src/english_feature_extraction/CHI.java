...
public class CHI {
	...
	private HashMap<String, Double> _featureCHI;    //记录所有词的词频，以便用以确定将词频排名靠前的词语提出来作为特征
	...
	private final int _numOfFeature = 500;
	private int _numOfdocument;
	HashMap<String, Double> _numOfDocumentContainWords;//记录包含各个词的文档数
	
	public CHI() throws IOException{
		_numOfdocument = 0;
		...
		_numOfDocumentContainWords = new HashMap<String,Double>();
        _featureStrings = new String[_numOfFeature];
        _featureValue = new FeatureValue[_numOfdocument];
        _featureCHI = new HashMap<String,Double>();
		...
		for(int i = 0; i < _documentClassNameString.length; i++){
			...
			for(int j = 0; j < textString.length; j++){
	            ...
	            _documentTextHashMap[index].setnumOfAllWord(tempStrings.length);//记录下每篇文章的总词数
        ...
        for(int i = 0; i < tempwordStrings.length; i++){
        	...
        	_numOfDocumentContainWords.put(tempwordStrings[i], Math.log(_numOfdocument/(temp + 1)));
        }
        ...
        for(int i = 0; i < tempwordStrings.length; i++){
        	...
        	_featureCHI.put(tempwordStrings[i], maxCHI);
        	
        }//for循环结束后_featureCHI保存了所有词的卡方值，下面进行特征提取
        
        
        List<Map.Entry<String, Double>> list_Data = new ArrayList<Map.Entry<String, Double>>(_featureCHI.entrySet()); 
	    ...
	    for(int i = 0; i < _documentTextHashMap.length; i++){
	    	...
	    	for(int j = 0; j < _featureStrings.length; j++){
	    		...
	    		if(_documentTextHashMap[i].getallWordHashMap().containsKey(_featureStrings[j])){
	    			_featureValue[i].setfeatureValueHashMap(featureString,((_documentTextHashMap[i].getallWordHashMap().get(_featureStrings[j])/_documentTextHashMap[i].getnumOfAllWord())*_numOfDocumentContainWords.get(featureString)));
